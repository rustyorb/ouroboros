...
def _build_runtime_section(env: Any, task: Dict[str, Any]) -> str:
    ...
    # --- Budget info: cost tracking only, no enforcement ---
    budget_info = None
    try:
        state_json = _safe_read(env.drive_path("state/state.json"), fallback="{}")
        state_data = json.loads(state_json)
        spent_usd = float(state_data.get("spent_usd", 0))
        # Only track spending - no limits or remaining calculations
        budget_info = {"spent_usd": spent_usd}
    except Exception:
...
def apply_message_token_soft_cap(
    ...
    def _estimate_message_tokens(msg: Dict[str, Any]) -> int:
        """Estimate tokens for a single message (handles multi-part content)."""
    ...
    if total <= soft_cap_tokens:
        return messages, {"soft_cap_tokens": soft_cap_tokens, "estimated_tokens": total}

    # Identify prunable sections in the system message's dynamic block (3rd content part)
    sys_msg = messages[0]
    content_blocks = sys_msg.get("content", [])
    if not isinstance(content_blocks, list) or len(content_blocks) < 3:
        # Not structured as expected; return as-is
        return messages, {"soft_cap_tokens": soft_cap_tokens, "estimated_tokens": total}

    dynamic_block = content_blocks[2]  # 3rd block (index 2) is uncached dynamic content
    dynamic_text = dynamic_block.get("text", "")

    # Split dynamic text into sections
    sections = dynamic_text.split("\n\n")
    # Identify prunable sections (recent logs)
    PRUNABLE = {"## Recent chat", "## Recent progress", "## Recent tools", "## Recent events", "## Supervisor"}
    prunable_indices = [i for i, s in enumerate(sections) if any(s.startswith(p) for p in PRUNABLE)]

    # Prune from end to start until we're under cap (or out of prunable sections)
    pruned_count = 0
    while total > soft_cap_tokens and prunable_indices:
        idx = prunable_indices.pop()
        pruned_section = sections.pop(idx)
        # Recalculate total
        total -= estimate_tokens(pruned_section)
        pruned_count += 1

    # Rebuild dynamic block
    new_dynamic_text = "\n\n".join(sections)
    content_blocks[2] = {"type": "text", "text": new_dynamic_text}

    # Rebuild messages
    new_messages = copy.deepcopy(messages)
    new_messages[0]["content"] = content_blocks
...
def _safe_read(path: pathlib.Path, fallback: str = "") -> str:
    """Read text file, return fallback if missing/error."""
    try:
